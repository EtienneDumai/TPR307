...
-- Question 11
SELECT P.Plnom, P.Plprenom, T.Volnum FROM PERSONNEL P
JOIN TRAJETS T ON P.Plnum = T.Plnum
GROUP BY P.Plnom, P.Plprenom, T.Volnum;

-- Question 12
SELECT DISTINCT A.Av_num, A.Avnom 
FROM AVION A
JOIN TRAJETS T ON A.Av_num = T.Av_num;

-- Question 13
SELECT COUNT(Volnum) FROM TRAJETS;

-- Question 14
SELECT COUNT(Capacite) FROM AVION;

-- Question 15
SELECT AVG(T.HEUREARR - T.HEUREDEP) FROM TRAJETS T;

-- Question 16
SELECT MIN(A.CAPACITE), MAX (A.CAPACITE) FROM AVION A;

-- Question 17
SELECT  SUM(HEUREARR-HEUREDEP) AS heuresDeVol FROM TRAJETS T
JOIN PERSONNEL P ON P.PLNUM = T.PLNUM
GROUP BY P.PLNUM;